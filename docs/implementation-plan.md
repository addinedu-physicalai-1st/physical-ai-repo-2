...
| 참가자 확정 | 진입 직후 | 현재 위치에서 GogoPing 노트북 웹캠 + 얼굴 인식 으로 등록 임베딩과 매칭해 시야 내 등록 아이를 후보로 표시, 교사 UI "시작" 클릭으로 참가 아이 (최대 5명) 확정 | hello | 위치 이동 |
| 위치 이동 | 참가자 확정 | 자율 주행 으로 놀이 위치 (`play_area`, UI 라벨 "운동장2") 로 이동 | basic | 카운트다운 |
| 카운트다운 | 놀이 위치 도착 | GogoPing 노트북 디스플레이의 셰이더 로봇 눈 (눈 가리기 표정) + 음성 합성 으로 30초 카운트다운, 그 동안 "꼭꼭 숨어라 머리카락 보인다" 반복 외침 | fun | 순찰 |
| 순찰 | 카운트다운 종료 | 자율 주행 으로 무작위 `patrol_*` named pose 순찰 | interest | (사람 감지) 호명 / (모든 patrol 소진) 복귀 |
| 호명 | 시야 내 사람 감지 (얼굴 인식) | 음성 합성 으로 "얼굴 보여주세요" 안내 → 얼굴 매칭 결과가 등록 참가 아이면 자동으로 "잡힘" 처리 후 순찰 재개, 미등록·매칭 실패면 순찰 재개 | happy | (남은 아이) 순찰 / (모두 잡힘) 복귀 |
| 복귀 | 모든 참가 아이 발견 / 모든 patrol 소진 | 자율 주행 으로 놀이 위치 (`play_area`) 로 복귀하며 도착할 때까지 "못찾겠다 꾀꼬리" 반복 외침 | basic | 종료 |
| 종료 (전역 트리거) | 복귀 도착 / 타임아웃 / 교사 종료 명령 (호출어 + 자연어 / UI) | 잡히지 않은 아이를 "오늘의 우승자" 로 발표하는 음성 + UI 카드, 대기 모드로 전환 | (우승자 있음) happy / (모두 잡힘) basic / (교사 종료) basic | 대기 |
