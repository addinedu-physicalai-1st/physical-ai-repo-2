...
| S ID | Name | Description | Priority |
| --- | --- | --- | --- |
| SR-PLAY-007 | 숨바꼭질 | GogoPing 이 숨바꼭질 모드에 진입해 운동장2 이동 → UI 모집 (얼굴 자동 등록) → 30초 카운트다운 → 전 웨이포인트 순회 (순찰 중 연속 얼굴 인식) → 복귀 (복귀 중 연속 얼굴 인식) → 우승자 발표 순으로 게임을 진행한다. UI 는 무궁화꽃이 피었습니다 모집 화면 패턴을 재활용. | High |

#### 단계 머신

| 단계 | 트리거 | 동작 | 표정 | 다음 단계 |
| --- | --- | --- | --- | --- |
| 진입 | §8.2 명령 인터페이스의 모드 전환 | 모드 진입, UI 가 숨바꼭질 화면 마운트 | hello | 위치 이동 |
| 위치 이동 | 진입 직후 | 자율 주행 으로 놀이 위치 (`play_area`, 운동장2) 로 이동. UI 는 "운동장2 로 가는 중" 화면 표시. | basic | 모집 |
| 모집 | 놀이 위치 도착 | GogoPing UI 가 등록 어린이 명단 카드(무궁화 entry 와 동일 패턴)를 띄우고, GogoPing 노트북 웹캠 + 얼굴 인식 으로 시야 내 등록 어린이를 자동 등록(최대 5명). 교사가 "출발" 누르거나 일정 시간 경과 시 종료. | hello | 카운트다운 |
| 카운트다운 | 모집 종료 (등록 1명 이상) | GogoPing 노트북 디스플레이의 셰이더 로봇 눈 (눈 가리기 표정) + 음성 합성 으로 30초 카운트다운. 카운트다운 동안 "꼭꼭 숨어라 머리카락 보일라!" 챈트를 일정 주기로 반복 재생. | fun | 순찰 |
| 순찰 | 카운트다운 종료 | 자율 주행 으로 모든 `patrol_*` named pose 를 정해진 순서로 한 번씩 순회. 각 노드 도착 시 PT 카메라 회전 (좌→우 스윕) 으로 시야 확보. **연속 얼굴 인식 활성** — 카메라 view 매 frame 얼굴 검출 + 등록 child_id 매칭, 등록자 발견 시 즉시 음성 호명 + caught API + UI 토스트 (정지 안 함). 노드 처리 후 다음 노드로 이동. | interest | (모든 노드 처리 완료) 복귀 / (모두 잡힘) 복귀 |
| 복귀 | 모든 `patrol_*` 노드 처리 완료 또는 모두 잡힘 | 음성 합성 으로 "못 찾겠다 꾀꼬리!" (모두 잡힌 경우엔 안내 음성) → 자율 주행 으로 운동장2 복귀. **연속 얼굴 인식 활성** — 순찰과 동일 파이프라인 유지, 복귀 중 발견자도 caught 처리. | (모두 잡힘) happy / (남은 아이 있음) sad | 발표 |
| 발표 | 운동장2 복귀 도착 | UI 가 우승자(아직 잡히지 않은 등록 참가자) 카드를 메달 표시와 함께 발표. 잡힌 아이는 잡힌 위치와 함께 별도 섹션에 표시. "다시 하기" / "대기로" 버튼 제공. | happy | 대기 |
| 종료 (전역 트리거) | 어느 단계에서든 교사 종료 명령 (호출어 + 자연어 / UI 닫기) | 진행 동작 종료 → 대기 모드로 전환 | basic | 대기 |
...
| key | 위치 | 사용 SR |
| --- | --- | --- |
| play_area | 숨바꼭질 놀이 위치 (= 운동장2 — 모집/카운트다운/복귀 도착점) | SR-PLAY-007 |
| patrol_1 .. patrol_N | 순찰용 위치 (N ≥ 3 권장). 숨바꼭질 순찰 단계에서 정해진 순서로 모든 노드를 한 번씩 순회 | SR-PLAY-007 |
...
| SR-PLAY-014 | ACT 체크포인트 사전 점검·다운로드 | NoriArm UI 부트스트랩 스크립트 (Robot UI `VITE_ROBOT=noriarm` 인스턴스 launcher) 가 시작 시 블럭쌓기 ACT 체크포인트 (`https://huggingface.co/jisoo3/act_game_block_stacking_0521`) 가 로컬 캐시 (`~/.cache/huggingface/hub/`) 에 존재하는지 확인하고, 없으면 `huggingface_hub.snapshot_download(repo_id="jisoo3/act_game_block_stacking_0521")` 로 다운로드한다. 진행률·완료 여부는 NoriArm UI 상단 배너로 표출하며, 다운로드 실패 시 블럭쌓기 모드 진입을 차단하고 에러 안내를 표시한다. | High |
