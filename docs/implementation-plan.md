...
| 단계 | 트리거 | 동작 | 표정 | 다음 단계 |
| --- | --- | --- | --- | --- |
| 진입 | §8.2 명령 인터페이스의 모드 전환 | 모드 진입, UI 가 숨바꼭질 화면 마운트 | hello | 모집 |
| 모집 | 진입 직후 | GogoPing UI 가 등록 어린이 명단 카드(무궁화 entry 와 동일 패턴)를 띄우고, GogoPing 노트북 웹캠 + 얼굴 인식 으로 시야 내 등록 어린이를 자동 등록(최대 5명). 교사가 "출발" 누르거나 일정 시간 경과 시 종료. | hello | 위치 이동 |
| 위치 이동 | 모집 종료 (등록 1명 이상) | 자율 주행 으로 놀이 위치 (`play_area`, 운동장2) 로 이동 | basic | 카운트다운 |
| 카운트다운 | 놀이 위치 도착 | GogoPing 노트북 디스플레이의 셰이더 로봇 눈 (눈 가리기 표정) + 음성 합성 으로 30초 카운트다운. 카운트다운 동안 "꼭꼭 숨어라 머리카락 보일라!" 챈트를 일정 주기로 반복 재생. | fun | 순찰 |
| 순찰 | 카운트다운 종료 | 자율 주행 으로 모든 `patrol_*` named pose 를 정해진 순서로 한 번씩 순회. 각 노드 도착 시 PT 카메라 회전 (좌→우 스윕) 으로 시야 확보. 시야 내 등록 참가 아이 발견 시 음성 합성 으로 이름 호명하고 해당 아이를 "잡힘" 으로 우승 후보에서 제외. 노드 처리 후 다음 노드로 이동. | interest | (모든 노드 처리 완료) 복귀 / (모두 잡힘) 복귀 |
| 복귀 | 모든 `patrol_*` 노드 처리 완료 또는 모두 잡힘 | 음성 합성 으로 "못 찾겠다 꾀꼬리!" (모두 잡힌 경우엔 안내 음성) → 자율 주행 으로 운동장2 복귀 | (모두 잡힘) happy / (남은 아이 있음) sad | 발표 |
