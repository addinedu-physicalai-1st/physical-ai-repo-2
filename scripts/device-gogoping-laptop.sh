  ...
  up|"")
    ...
    if [[ -n "${ROS_PEER_HOST:-}" ]] && [[ -f "$MACHINE_IPS" ]] && command -v jq &>/dev/null; then
      ...
      if [[ ${#_peer_ips[@]} -gt 0 ]]; then
        ...
        # 다른 인터페이스 (docker bridge 172.x 등) 가 locator 로 announce 되지 않도록 whitelist.
        ...
        echo "[device-gogoping-laptop] Fast DDS profile: $_xml_path (peers: ${_peer_ips[*]}, port: $_port, my_ip: ${_my_ip:-(not whitelisted)})"
      fi
    fi
