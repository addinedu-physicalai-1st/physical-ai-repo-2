...
한국어 mode 이름 ("추종", "이동", "수동", "자장가", "숨바꼭질", "대기") 을
...
장소로 이동할지). 데모 기본값은 ``DEMO_*`` 상수로 격리해 후속 수정 명확하게.
...
def mode_to_goal(mode_label: str) -> Goal:
    ...
    ------
    UnsupportedMode
        gogoping modes (대기/보조/추종/이동/자장가/놀이/숨바꼭질/수동) 에 없는 라벨.
    ...
    if mode_label in ("보조", "놀이"):
        raise UnsupportedMode(
            ...
            f"하위 모드 (추종/이동/자장가 / 숨바꼭질) 를 사용하세요."
        )
