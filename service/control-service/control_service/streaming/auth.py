...
[service/control-service/control_service/auth.py](../auth.py) 의 cookie_backend 와 같은 cookie 이름·DB 전략.
...
from control_service.config import settings as control_settings
from control_service.streaming.config import settings as streaming_settings
from control_db.models import AccessToken
...
async def _validate_token(token: str) -> Optional[str]:
    """[service/control-service/control_service/auth.py](../auth.py) 의 cookie_transport 와 같은 cookie name."""
    try:
