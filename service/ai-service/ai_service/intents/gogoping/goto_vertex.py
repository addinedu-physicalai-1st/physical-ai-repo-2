...
추가 기능:
- vertex alias — 자연어 구절을 특정 vertex 로 매핑 (현재 비어 있음 — 운동장 제거됨).
  vertex 이름을 직접 안 말해도 이동. (별칭은 goto 패턴 없이도 트리거 — 구절 자체가 이동 의도)
...
# (운동장 제거됨 — '밖으로 나가' 류 별칭 폐기. 새 별칭 필요 시 여기 추가.)
...
_VERTEX_NAME_ALIASES: dict[str, str] = {
    # 놀이방 — 받침/유사음/띄어쓰기 변형 (Whisper 가 짧은 방 이름을 흔히 흘리는 형태).
    "놀이반": "놀이방",
    "놀이빵": "놀이방",
    "놀이팡": "놀이방",
    "놀이망": "놀이방",
...
_THEN_MODE_KEYWORDS: dict[str, str] = {
    # 자장가 — 복합 명령 "X 가서 자장가" 의 도착 후 전환. STT 변형도 흡수.
    "자장가": "자장가",
    "자장까": "자장가",
    "자장노래": "자장가",
    "자장 가": "자장가",
