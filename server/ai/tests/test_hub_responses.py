...
import server.ai.hub as hub_mod
from server.ai.hub import app
...
def test_voice_intent_name_question_uses_chat_path() -> None:
    with patch("server.ai.hub.generate_chat", new_callable=AsyncMock) as m:
...
def test_voice_intent_llm_timeout_returns_teacher_line() -> None:
    ...
    with patch.object(hub_mod.ai_settings, "voice_chat_llm_max_wait_s", 0.1):
        with patch("server.ai.hub.build_chat_context", new_callable=AsyncMock) as _ctx:
            _ctx.return_value = {}
            with patch(
                "server.ai.hub.fetch_registered_children_labels",
                new_callable=AsyncMock,
            ) as _ro:
                _ro.return_value = None
                with patch("server.ai.hub.generate_chat", side_effect=slow_chat):
                    r = client.post(
                        "/voice/intent",
...
def test_voice_intent_lunch_fast_path_mocked_menu() -> None:
    with patch("server.ai.context.get_menu_fast", new_callable=AsyncMock) as m:
