...
_HALLUCINATION_PATTERNS = [
    ...
    re.compile(r"무엇인지\s*잘\s*모르겠"),
    re.compile(r"메뉴가\s*나오지\s*않았"),
]
...
async def generate_chat(
    ...
    try:
        ...
        # JSON이 아니면 텍스트만이라도 추출 시도
        m = _REPLY_RE.search(raw)
        if m:
            data = {"reply": m.group(1), "emotion": "basic"}
        else:
