...
``servo_bridge`` 노드의 ``/servo_bridge/cmd_pan`` · ``/servo_bridge/cmd_tilt``
(std_msgs/Float32 deg) 로 setpoint publish. servo_bridge 가 clamp + rate_limit 후
시리얼로 Arduino 에 전달.
...
# servo_bridge 의 ~/cmd_pan · ~/cmd_tilt — 절대 path. params.yaml 토픽과 일치.
TOPIC_CMD_PAN = "/servo_bridge/cmd_pan"
TOPIC_CMD_TILT = "/servo_bridge/cmd_tilt"

# 펌웨어/노드 양쪽 clamp 범위 (gogoping_camera_pan/CLAUDE.md · servo_bridge params 참조)
PAN_MIN_DEG = 5.0
PAN_MAX_DEG = 175.0
PAN_CENTER_DEG = 90.0
TILT_MIN_DEG = 30.0
...
class CameraPanClient:
    """카메라 pan/tilt 서보 setpoint publisher.

    생성 시 publisher 2개 (pan/tilt) 생성. ``publish_pan(deg)`` / ``publish_tilt(deg)``
    호출 시 각 축 범위로 clamp 후 publish.
    ...
    def publish_tilt(self, deg: float) -> float:
        """tilt setpoint publish. clamp(30~150) 적용 후 실제 publish 된 deg 반환."""
        from std_msgs.msg import Float32
        clamped = max(TILT_MIN_DEG, min(TILT_MAX_DEG, float(deg)))
        msg = Float32()
        msg.data = clamped
    ...
    def center(self) -> float:
        """pan 90° 복귀 — terminate/cleanup 시 호출."""
