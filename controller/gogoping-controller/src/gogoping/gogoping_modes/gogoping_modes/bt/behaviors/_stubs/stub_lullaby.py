...
## 진짜 lullaby sub tree 의 모양 (추후 작성)
...
이지만, 현재 단계엔 timer 처럼 종료.

## 교체 시점

진짜 트리는 ``WaitForStopCommand`` behavior (blackboard cancel 플래그 폴링)
...
class StubLullaby(StubRunningThenSuccess):
    ...
    # STUB — bt/trees/sub_trees/BT_lullaby_sub.py 의 build_lullaby 로 교체.
    """
