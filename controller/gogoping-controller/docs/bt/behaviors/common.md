...
외부 명령 수신 → blackboard 세팅 + `fsm.trigger()` 호출. **CHARGING/IDLE/GOTO/FOLLOW/LULLABY/HIDEANDSEEK/MANUAL/RETURNING MainTree 에 배치** (LOW_BATTERY_RETURNING/ERROR 는 의도적 제외 — lockdown).
...
| SetGoal 처리 | `utils/goal_reconciler.py` 의 순수 함수 호출 — 현재 state ↔ Goal 차이를 보고 적절한 `*_request` trigger 매핑. ERROR / LOW_BATTERY_RETURNING 진입 시 거부 (`accepted=False` + reason). |
| ForceState 처리 | `fsm.force_state(target_state)` 호출. transition 우회 — 디버그 전용. |
| Blackboard write | `target_id`, `destination_key`, `search_waypoints`, ... |
| FSM trigger | `goto_request` / `follow_request` / `lullaby_request` / `hideseek_request` / `manual_request` / `return_request` / `cancel` (reconciler 가 매핑) |
| Used in | BT_charging_main, BT_idle_main, BT_goto_main, BT_follow_main, BT_lullaby_main, BT_hide_and_seek_main, BT_manual_main, BT_returning_main |
...
| Used in | BT_idle_main, BT_goto_main, BT_follow_main, BT_lullaby_main, BT_hide_and_seek_main, BT_returning_main (RETURNING → LOW_BATTERY_RETURNING escalation). **MANUAL 의도적 제외** — 사용자 직접 제어 중 자동 빼앗김 방지 |
...
**배치**: CHARGING / IDLE / GOTO / FOLLOW / LULLABY / HIDEANDSEEK / RETURNING / LOW_BATTERY_RETURNING (**8 트리**). **MANUAL/ERROR 제외** — MANUAL 은 사용자 직접 제어 중 자동 ERROR 차단 (battery/HW/collision 일관 정책). ERROR 는 terminal.

**향후 확장**:
- ZLAC 모터 통신 끊김 — vicpinky_bringup 이 `/gogoping/motor_health` (`diagnostic_msgs`) publish 하면 추가
- IMU staleness — IMU 도입 시 추가

| Used in | 8 트리 (CHARGING/IDLE/GOTO/FOLLOW/LULLABY/HIDEANDSEEK/RETURNING/LOW_BATTERY_RETURNING) |
...
- 발화 후에는 `_fired = True` 로 edge-triggered 유지 (반복 발화 방지). ERROR terminal 이라 re-arm 필요 없으나 `initialise()` 가 새 트리 진입 시 리셋해주므로 GOTO/FOLLOW/LULLABY/HIDEANDSEEK/RETURNING 재진입 시 다시 발화 가능.
...
| Used in | **9 트리** — BT_charging_main, BT_idle_main, BT_goto_main, BT_follow_main, BT_lullaby_main, BT_hide_and_seek_main, BT_manual_main, BT_returning_main, BT_low_battery_returning_main (ERROR 만 제외 — terminal). **MANUAL 예외** — battery/hw/collision 은 MANUAL 미배치지만 MapBoundaryMonitor 만 예외적 배치 (위치 안전 우선) |
