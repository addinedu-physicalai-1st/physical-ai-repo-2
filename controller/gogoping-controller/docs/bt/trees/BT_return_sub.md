...
└─ Sequence (memory=True)
     ...
     └─ ReverseIntoDock                                              # N초 후진 → SUCCESS
```

`OneShot(ON_COMPLETION)` 으로 감싸 한 번 SUCCESS / FAILURE 후 재실행 차단 (Sequence 가 매 tick 다시 처음부터 돌아 NavigateToVertex 가 도크에서 다시 나가는 버그 방지).
...
| (종료) `docked` | **사람** (admin UI 디버그 버튼) | RETURNING → CHARGING. 자동 도킹 (VerifyDockingContact) 은 발표 범위 외 |
...
- VerifyDockingContact / 자동 `docked` trigger
- `docking_contact` 토픽 / 센서 통합
