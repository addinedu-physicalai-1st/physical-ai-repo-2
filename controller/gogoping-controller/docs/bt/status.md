...
| Admin UI MapStatusCard (좌표 + IN/OUT) | ✅ | ODOM 카드 옆 — 배지 (🟢 IN MAP / 🔴 OUT OF MAP / ⚪ UNKNOWN) + 좌표 (x/y/yaw) + 디버그 좌표 override 입력 (적용 / 원복). [widgets/map_status_card.py](../../../../app/admin-app/widgets/map_status_card.py) |
| Debug 좌표 override (SetRobotPose.srv) | ✅ | admin UI 입력 → POST `/api/gogoping/debug/pose` → ros_bridge.set_robot_pose_sync → command_listener server. blackboard.ROBOT_POSE 강제 (map frame) + POSE_OVERRIDE_ACTIVE=True (PoseSubscriber 가 amcl_pose 메시지 무시). clear=True 로 live amcl_pose 복원. 4 시나리오 통과 |
