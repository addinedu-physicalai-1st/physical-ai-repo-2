...
| `cancel` | — | `command_listener` | ASSIST / PLAY / MANUAL / **RETURNING** → IDLE | task 키 리셋. MANUAL 에서 발화 시 torque ON 자동. RETURNING 에서 발화 시 사용자 "복귀 취소" — BT_return_sub OneShot 의 `terminate()` 가 cmd_vel=0 정리. LOW_BATTERY_RETURN 은 lockdown 제외 |
...
```
   ...
   CHARGING ──battery_full──▶ IDLE ◀── assist_done / play_done / cancel
       ▲                       │
       │ docked                │ assist_request / play_request / manual_request (IDLE/active/RETURNING → active)
...
> **다중 source 전이**
> - `assist_request` (**IDLE / PLAY / MANUAL / RETURNING → ASSIST**): active mode 간 직접 전이 + RETURNING 도중 사용자가 마음 바꿔 ASSIST 명령 — BT swap 1회로 처리. LOW_BATTERY_RETURN 은 lockdown 제외
> - `play_request` (**IDLE / ASSIST / MANUAL / RETURNING → PLAY**): 동일
> - `manual_request` (**IDLE / ASSIST / PLAY / RETURNING → MANUAL**): 동일. MANUAL 진입 시 `ManualTorqueHold.initialise()` 가 torque OFF
...
> - `cancel` (ASSIST / PLAY / MANUAL / **RETURNING** → IDLE): MANUAL 의 정상 종료 경로 (torque 자동 ON). RETURNING 에서 발화 시 사용자 "복귀 취소" — BT_return_sub OneShot 의 `terminate()` 가 cmd_vel=0 정리
> - `docked` (RETURNING / LOW_BATTERY_RETURN → CHARGING): BT_return_sub Sequence 의 마지막 자식 `VerifyDockingContact` 가 ReverseIntoDock 완료 직후 자동 발사
>
